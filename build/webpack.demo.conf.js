...
function parseRef(str) {
	const result = str[1].match(
		/([@_a-zA-Z]+)(\s+|=|$)("[^"]+"*|\d+|true|false)?/g,
	)
	const ret = {}
	if (result.length) {
		result.forEach(item => {
								...
								[
									...
									'demo',
									{
										...
										render(tokens, idx) {
											...
											const m =
												tokens[idx].info.trim().match(/^demo\s*(.*)$/) || ''
										...
										render: function render(tokens, idx) {
											...
											if (curToken.nesting === 1) {
												...
												if (params.src) {
													...
													throw new Error(
														'include container must has src param',
													)
												}
											}
											return '</div></demo-block>\n'
							...
							preprocess: function preprocess(MarkdownIt, source) {
								...
								MarkdownIt.renderer.rules.link_open = function (
									...
									if (/^[http|https]/.test(href) || /^\/\//.test(href)) {
										...
										// relative path
										const to =
											publicPath !== '/' ? `${publicPath}${href}` : href
										token.attrSet('herf', '')
								...
								const matches =
									source.match(/:::\s*include\s*\(([^()]*?)\)[\s\S]*?:::/g) ||
									''
								const scripts = []
								const components = []
								if (matches) {
